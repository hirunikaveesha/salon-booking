...
if (strlen($_SESSION['bpmsuid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_POST['services']) && is_array($_POST['services'])) {
            ...
            $services = ''; // If no services are selected
        }

        $query = mysqli_query($con, "INSERT INTO tblbook(UserID,AptNumber,AptDate,AptTime,Message,SelectedServices) 
                                     VALUES ('$uid','$aptnumber','$adate','$atime','$msg','$services')");
...
<body id="home">
        ...
        <div class="about-inner contact ">
            <div class="container">
                <div class="main-titles-head text-center">
                    ...
                    <p class="tiltle-para">Indulge in a pampering experience! Reserve your moment of relaxation and
                        beauty with our easy-to-use online booking system. Elevate your style at our salon today!</p>
                </div>
            </div>
        </div>
                ...
                <div class="d-grid contact-view">
                    ...
                    <div class="map-content-9 mt-lg-0 mt-4">
                        <form method="post">
                            ...
                            <button type="submit" class="btn btn-contact" name="submit">Make an
                                Appointment</button>
                        </form>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#selected_services_list").hide();
            $("input[name='services[]']").change(function () {
                var selectedServices = [];
                $("input[name='services[]']:checked").each(function () {
                    selectedServices.push($(this).next('span').text());
                });

                var selectedServicesList = '';
                for (var i = 0; i < selectedServices.length; i++) {
                    selectedServicesList += '<span class="selected-service">' + selectedServices[i] + '</span> ';
                }

                $("#selected_services_list").html(selectedServicesList).show();
            });

            // Handle removing a selected service on click
            $(document).on('click', '.selected-service', function () {
                var serviceToRemove = $(this).text().trim();
                $("input[name='services[]'][value='" + serviceToRemove + "']").prop('checked', false);
                $(this).remove();
                if ($("#selected_services_list").is(':empty')) {
                    $("#selected_services_list").hide();
                }
            });
